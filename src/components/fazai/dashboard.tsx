...
export function Dashboard() {
  ...
  return (
    <div className="flex flex-col gap-4 pb-20">
      ...
      >
        <Card className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white p-6 rounded-2xl shadow-lg border-0">
          <p className="text-sm opacity-90">{t('dash.balance', lang)}</p>
          <p className="text-3xl font-bold mt-1">{formatNumber(balance)}</p>
          <div className="flex gap-6 mt-4">
            <div className="flex items-center gap-1.5">
              <ArrowUpRight className="w-4 h-4 text-green-200" />
              <div>
                <p className="text-[10px] opacity-80">{t('dash.income', lang)}</p>
                <p className="text-sm font-semibold">{formatNumber(todayIncome)}</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              ...
              <div>
                ...
                <p className="text-sm font-semibold">{formatNumber(todayExpense)}</p>
              </div>
            </div>
          </div>
        </Card>
      ...
      <div className="grid grid-cols-2 gap-3">
        <motion.button
          ...
          className="flex flex-col items-center justify-center gap-2 bg-gradient-to-br from-green-500 to-emerald-600 text-white rounded-2xl p-6 shadow-md min-h-[120px] active:scale-95 transition-transform"
        >
          <TrendingUp className="w-8 h-8" />
          <span className="text-lg font-bold">{t('dash.income', lang)}</span>
        </motion.button>
        <motion.button
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          onClick={() => navigate('expense')}
          className="flex flex-col items-center justify-center gap-2 bg-gradient-to-br from-red-500 to-rose-600 text-white rounded-2xl p-6 shadow-md min-h-[120px] active:scale-95 transition-transform"
        >
          <TrendingDown className="w-8 h-8" />
          <span className="text-lg font-bold">{t('dash.expense', lang)}</span>
        </motion.button>
      </div>
